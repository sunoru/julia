...
@testset "Pauli σ-matrices" for
    σ in map(Hermitian, Any[ eye(2), [0 1; 1 0], [0 -im; im 0], [1 0; 0 -1] ])
...
@testset "Core functionality" begin
    ...
    areal = randn(n,n)/2
    aimg  = randn(n,n)/2
    @testset "symmetric eigendecomposition. eltype(a)=$(eltya)" for
        eltya in (Float32, Float64, Complex64, Complex128, BigFloat, Int)
...
@testset "#Issue #7647: test xsyevr, xheevr, xstevr drivers. Eigenvalues in interval for $(typeof(Mi7647))" for
    Mi7647 in (Symmetric(diagm(1.0:3.0)),
...
@testset "Issues #8057 and #8058. f=$f, A=$A" for
    f in (eigfact, eigvals, eig),
...
@testset "Unary minus for Symmetric matrices" begin
