...
const SA_CTR = Ref(1)
next_sa_ctr() = (id = SA_CTR[]; SA_CTR[] = id+1; id)

mutable struct SA_ID
    whence::Int
    id::Int

    SA_ID() = SA_ID(myid(), next_sa_ctr())
    SA_ID(whence, id) = new(whence,id)
end
hash(r::SA_ID, h::UInt) = hash(r.whence, hash(r.id, h))
==(r::SA_ID, s::SA_ID) = (r.whence==s.whence && r.id==s.id)
...
mutable struct SharedArray{T,N} <: DenseArray{T,N}
    ...
    function SharedArray{T,N}(d,p,r,sn,s) where {T,N}
        S = new(SA_ID(),d,p,r,sn,s,0,view(Array{T}(ntuple(d->0,N)), 1:0))
        sa_refs[S.id] = WeakRef(S)
        S
    end
end

const sa_refs = Dict{SA_ID, WeakRef}()
...
function deserialize(s::AbstractSerializer, t::Type{<:SharedArray})
    ...
    if ref_exists
        sref = sa_refs[SA_ID(deserialize(s), deserialize(s))]
        if sref.value !== nothing
            return sref.value
        else
            throw(ErrorException("Expected reference to shared array instance not found"))
        end
    end
